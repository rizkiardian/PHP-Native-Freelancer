...
if (!empty($selectedCategory) && $_GET['kategori'] !== 0) {
    $sql .= "  WHERE kr.id = $selectedCategory AND pr.status = 'selesai'";
}else{
    $sql .= " WHERE pr.status = 'selesai'";
}

$kategori = mysqli_query($conn, "SELECT * FROM `kategori_request` WHERE id = $selectedCategory");
                    ...
                    <div class="col">
                        <div class="project-wrapper d-flex flex-column shadow-sm">
                            <div class="tgl-invoice d-flex flex-row justify-content-between pt-3 px-3">
                                <p class="invoice"><?= $project['invoice'] ?></p>
                                <p class="tgl-pesanan">Pesanan diterima: <?=date('d M Y', strtotime($project['tglTerima']));?></p>
                            </div>
                            <div class="content-main d-flex flex-row mt-3 px-4">
                                ...
                                <div class="namaFreelancer ms-4">
                                    <h4 class="mb-2"><?= $project['freelancer'] ?></h4>
                                ...
                                <div class="d-flex flex-column project ms-4">
                                    ...
                                    <div class="row gx-5">
                                        ...
                                        <div class="col-5 d-flex flex-column justify-content-evenly judul-project ms-5">
                                            ...
                                            <a href="beriulasan.php?id=<?= $project['idProject'] ?>" class="d-flex justify-content-center align-items-center">Beri ulasan</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
